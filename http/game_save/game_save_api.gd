...
func update_game_save_currencies(
	game_save_id: String, gold: int, diamonds: int, emeralds: int, amethysts: int, on_failure: Callable
) -> bool:
	var body: Dictionary = {}
	...
	var response: HTTPResult = await Http.api_client.post(
		Http.api_routes.UPDATE_GAME_SAVE_CURRENCIES.format({"game_save_id": game_save_id}), true, body
	)
